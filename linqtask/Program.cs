...
using linqtask;
    ...
    List<Debtor> debtors = new List<Debtor> {
        new Debtor("Shirley T. Qualls", DateTime.Parse("March 30, 1932"), "530-662-7732", "ShirleyTQualls@teleworm.us", "3565 Eagles Nest Drive Woodland, CA 95695", 2000),
        ...
        new Debtor("Charles P. Segundo", DateTime.Parse("October 21, 1970"), "415-367-9341", "CharlesPSegundo@dayrep.com", "1824 Roosevelt Street Fremont, CA 94539", 5648),
    ...
    //     (dpt) => dpt.Email.EndsWith("rhyta.com") || dpt.Email.EndsWith("dayrep.com"));
...
// var result = debtors.Where(
//     (dpt) => DateTime.Now.Year - dpt.BirthDay.Year > 26 && DateTime.Now.Year - dpt.BirthDay.Year > 36);
// foreach (var item in result)
//     Console.WriteLine(item);
...
// var result = debtors.Where(
// (dpt.FullName.Length > 18 && dpt.Phone.Where((herf) => herf == '7')));
// foreach (var item in result)
//     Console.WriteLine(item);
// 
...
// var result = debtors.Where(
// (dpt) => dpt.BirthDay.Month == 12 || dpt.BirthDay.Month == 1 || dpt.BirthDay.Month == 2);
// foreach (var item in result)
//        Console.WriteLine(item);
...
// var result = debtors.Where(debtor => debtor.Debt <= averageDebt).ToList();
...
//     var result = debtors
// .Where(dpt => dpt.Phone.Contains("8"))
// .Select(dpt => new
// {
//     Name = dpt.FullName,
//     Age = DateTime.Today.Year - dpt.BirthDay.Year,
//     dpt.Debt
// });
// foreach (var item in result)
// {
...
//             .OrderBy(fullName => new string(fullName
...
// var result = debtors.Select(dbt => dbt.Debt).Sum();
// Console.WriteLine(result);

// 16)2ci dunya muharibesin gormush borclularin siyahisi cixartmaq
// var result = debtors.Where(dpt => dpt.BirthDay.Year <= 1939 || dpt.BirthDay.Year <= 1945);
// foreach (var item in result)
//     Console.WriteLine(item);

// 18)Nomresinde tekrar reqemler olmayan borclularin ve onlarin borcunun meblegin cixartmaq
// var result = debtors
//             .Where(debtor => debtor.Phone.Distinct().Count() == debtor.Phone.Length).Select(dpt => dpt.Debt);
...
// var result = debtors
//     .Select(debtor => {
//         int monthDifference = Math.Abs(debtor.BirthDay.Month - DateTime.Now.Month);
//         debtor.Debt -= monthDifference * 500;

//         return debtor;
//     })
//     .Where(debtor => debtor.Debt <= 0)
//     .ToList();
...
// var result = debtors.Where(debtor => debtor.FullName.Contains("s") && 
//                      debtor.FullName.Contains("m") && 
//                      debtor.FullName.Contains("i") && 
//                      debtor.FullName.Contains("l") && 
//                      debtor.FullName.Contains("e")).ToList();
// foreach (var item in result)
//     Console.WriteLine(item);
